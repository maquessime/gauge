...
func GenerateDoc(pluginName string, specDirs []string, port string) {
	...
	if !isPluginValidFor(pd, docsScope) {
	...
	os.Setenv(common.APIPortEnvVariableName, port)
	if err != nil {
		logger.Fatalf("Error setting environment for plugin %s %s. %s", pd.Name, pd.Version, err.Error())
	}

	p, e := StartPlugin(pd, docsScope)
	if e != nil {
